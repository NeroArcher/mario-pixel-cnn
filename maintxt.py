...
parser.add_argument('-d', '--dataset', type=str,
                    default='mario', help='Can be either cifar|mnist')
...
base_pathy = './VGLC/multi-layer/structural-layer'
tile_width = 10
tile_height = 10
verbose = False
my_x = []
...
class MyDataset(Dataset):
    def __init__(self, data, target datapath, transform=None):
        self.data = torch.from_numpy(data).float()
        self.target = torch.from_numpy(target).long()
        self.transform = transform
        #load x and y .. (self.x) + loop
...
for file_name in os.listdir(base_path): 
    ...
    my_x.append(res)
    my_y.append(torch.Tensor(res))

tensor_x = torch.FloatTensor(my_x) # transform to torch tensor
tensor_y = torch.FloatTensor(my_y)
my_dataset = MyDataset(tensor_x,tensor_y)
...
if 'mnist' in args.dataset : 
    ...
    test_loader  = torch.utils.data.DataLoader(my_dataset, batch_size=args.batch_size, shuffle=True, **kwargs)
